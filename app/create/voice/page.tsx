  ...
  return (
    <PageLayout className="flex flex-col gap-3 pt-8 pb-8 px-6">
      ...
      <div className="bg-white flex flex-col gap-5 items-center px-5 py-[30px] rounded-[10px] w-full">
        <div className="flex flex-col gap-1 items-start text-[#474741] text-[18px] font-semibold w-full">
          <p className="leading-6">고인의 목소리가 담긴 영상이나</p>
          <p className="leading-6">음성 파일을 올려주세요 (1분 이상)</p>
        </div>

        {!uploadedFile ? (
          <label className="bg-[#e0ded9] flex gap-[14px] items-start px-4 py-[22px] rounded-[6px] w-full cursor-pointer">
            <div className="bg-white flex items-center justify-center p-[6px] rounded-[4px] size-[40px] shrink-0">
              <RecordIcon />
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-[#474741] text-[16px] font-semibold tracking-[0.7px]">음성 파일 업로드</p>
              <p className="text-[#808080] text-[12px] font-medium tracking-[0.7px]">mp4, mov, m4a, wav 형식 가능해요</p>
            </div>
            <input type="file" accept="audio/*,video/*" className="sr-only" onChange={handleFileChange} />
          </label>
        ) : (
          <>
            <div className="bg-[#e0ded9] flex flex-col gap-2 items-start px-5 py-[22px] rounded-[6px] w-full">
              <div className="bg-white flex items-center justify-center p-[6px] rounded-[4px] size-[40px]">
                <RecordIcon />
              </div>
              <div className="flex flex-col gap-1 pt-1">
                <p className="text-[#474741] text-[16px] font-semibold tracking-[0.7px]">{uploadedFile.name}</p>
                <p className="text-[#808080] text-[14px] font-medium">업로드 완료</p>
              </div>

              <div className="flex flex-col gap-[10px] items-center py-[10px] w-full">
                <div className="relative bg-white h-[6px] rounded-[4px] w-full">
                  <div
                    className="absolute bg-[#775a19] h-[6px] left-0 rounded-l-[4px]"
            ...
            <label className="bg-[#e0ded9] flex flex-col gap-[6px] items-center justify-center p-4 rounded-[6px] w-full cursor-pointer">
              <AddIcon color="#808080" />
              <span className="text-[#808080] text-[14px] font-semibold tracking-[0.7px]">음성 파일 추가 업로드</span>
              <input type="file" accept="audio/*,video/*" className="sr-only" onChange={handleFileChange} />
            </label>
          </>
        )}
